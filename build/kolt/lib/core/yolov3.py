...
class TrtYOLOv3(object):
    ...
    def _load_engine(self):
        TRTbin = '/home/nvidia/catkin_build_ws/src/kolt_ros/src/core/yolov3_onnx/%s.trt' % self.model
    ...
    def __init__(self, model, input_shape=(416, 416)):
        ...
        # rospy.loginfo("inputs: {}".format(self.inputs))
        # rospy.loginfo("outputs: {}".format(self.outputs))
        # rospy.loginfo("stream: {}".format(self.stream))
        # check()

    # def check(self):
    #     rospy.loginfo("Current state of YOLOv3: {}".format(self.context))                                                                                                                                                  rospy.loginfo("inputs: {}".format(self.inputs))                                                                                                                                                                    rospy.loginfo("outputs: {}".format(self.outputs))
    #     rospy.loginfo("stream: {}".format(self.stream)) 
    def __del__(self):
        """Free CUDA memories."""
    ...
    def detect(self, img, conf_th=0.3):
        ...
        rospy.loginfo("Just tried to access self.output_shape")
        ...
        rospy.loginfo("Post processing finished")
        rospy.loginfo("YOLOV3 found {} boxes".format(len(boxes)))
